...
/*
//Using an interator, instead of making histograms, fill energy branch with clover infomation that exists in coicidence with the pg-timing conditions. 


std::map<int,double> add_energy; //int det#, double summed energy. 
std::map<int,int>    add_count; //int det# key, int update the amount of times we fill the detector.
std::map<int,double>::iterator it;
for(it = add_energy.begin();it!=add_energy.end();it++) {
if(it->first < 0){continue;}

if(it->first > 0){
// obj.FillHistogram(Form("Addback_%i",it->first), 8000,0,4000,it->second);
Energy[it->first] = it->second; 
...
int CLVID = find_clover_id(gmap[hit1.GetNumber()]);  //det 6<
int NCLV  = not_clover(gmap[hit1.GetNumber()]);        // det 7,8,9   
//Then all the crystal hits with the same id will be summed:        
add_energy[CLVID] += hit1.GetEnergy();
//Each crystal event gets recorded as well.             
add_count[CLVID] += 1;

for(int j = 1;j<=CLVID;j++){
Detid[j] = CLVID;  //Clovers 1 - 6

}
for(int n = 7;n<=NCLV;n++){
Detid[n] = NCLV;  //FSU7 and SB1&2
}
...
Time[CLVID] = gt ;    
Time[NCLV] = hit1.GetTime(); 

Theta[CLVID] = angle; 
// Theta[NCLV] = hit1.GetPosition(); 
}
//Using the available information about the different energies of each hit, we need to take the highest energy deposition for each clover and return the position of that hit. 


for(int r = 7; r<NCLV; r++){
Energy[r] = fsu->GetFSUHit(x).GetEnergy(); 
